    ...
    <div className='bg-[#f3f3f3] flex justify-center px-4 pt-6'>
            ...
            className='w-full max-w-6xl bg-white rounded-[24px] shadow-sm border border-gray-200 px-8 py-4 flex justify-between items-center relative'>
                    <div className='flex items-center gap-3 cursor-pointer'>
                            ...
                            <h1 className='font-semibold hidden md:block text-lg'>HirePrep AI</h1>
                    </div> 
                        <div className='flex items-center gap-6 relative'>
                            ...
                            <div>
                                    <button onClick={()=>{
                                        ...
                                        setShowUserPopup(!showUserPopup); setShowCreditPopup(false)} } className='w-9 h-9 bg-black text-white rounded-full flex items-center justify-center font-semibold'> {userData?.user?.name.slice(0,1).toUpperCase() || <FaUserAstronaut size={16} />} </button>

                                    {showUserPopup && (
                                        <div className='absolute right-0 mt-3 w-48 bg-white shadow-xl border border-gray-200 rounded-xl p-4 z-50'> 
                                            <p className='text-md text-blue-500 font-medium'> {userData?.user?.name} </p>
                                            ...
                                            <button onClick={handleLogout} className='w-full lext-left text-sm py-1 flex items-center gap-2 text-red-500'><HiOutlineLogout size={16}/>Logout</button>
                                        </div>
                                    )}
                            </div>
                        </div> 
